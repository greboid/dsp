...
- `POST /images/create`: Allowed only if `-allowPulls` is enabled
...
- `-allowPulls` (default: `false`): Allow pulling images via `POST /images/create`

Environment variables: `SOCKET`, `PROXYPORT`, `KILLSIGNALS`, `ALLOWPULLS`
